...
def sai_vlan_obj(npu):
    vlan_oid = npu.create(obj_type=SaiObjType.VLAN, attrs=["SAI_VLAN_ATTR_VLAN_ID", TEST_VLAN_ID])
    yield vlan_oid
...
def sai_vlan_member(npu, sai_vlan_obj):
    ...
    npu.set(oid=npu.port_oids[0], attr=["SAI_PORT_ATTR_PORT_VLAN_ID", TEST_VLAN_ID])
    yield vlan_mbr_oid, sai_vlan_obj
    npu.remove(oid=vlan_mbr_oid)
...
def test_set_attr(npu, dataplane, sai_vlan_obj, attr, attr_value):
    if attr == "SAI_VLAN_ATTR_STP_INSTANCE":
        status, data = npu.get(oid=npu.switch_oid, attrs=["SAI_SWITCH_ATTR_DEFAULT_STP_INST_ID", attr_value], do_assert=False)
        assert status == "SAI_STATUS_SUCCESS"
        attr_value = data.oid()

    status = npu.set(oid=sai_vlan_obj, attr=[attr, attr_value], do_assert=False)
    npu.assert_status_success(status)
...
def test_vlan_scaling(npu, dataplane, vlan_min, vlan_max):
    ...
    for vlan_id in range(vlan_min, vlan_max + 1):
        ...
        if vlan_id == TEST_VLAN_ID or vlan_id == npu.default_vlan_id:
            continue
        status, vlan_oid = npu.create(obj_type=SaiObjType.VLAN, attrs=["SAI_VLAN_ATTR_VLAN_ID", vlan_id], do_assert = False)
...
def test_vlan_create_negative(npu, dataplane, vlan_id, expected_status):
    ...
    status, vlan_oid = npu.create(obj_type=SaiObjType.VLAN, attrs=attrs, do_assert = False)
    if status == "SAI_STATUS_SUCCESS":
        npu.remove(oid=vlan_oid)
...
def test_vlan_member_set(npu, dataplane, sai_vlan_member, attr, attr_value):
    status = npu.set(oid=sai_vlan_member[0], attr=[attr, attr_value], do_assert=False)
    npu.assert_status_success(status)
...
def test_vlan_mbr_set_negative(npu, dataplane, sai_vlan_member, attr):
    ...
    if attr == "SAI_VLAN_MEMBER_ATTR_VLAN_ID":
        status = npu.set(oid=sai_vlan_member[0], attr=[attr, npu.default_vlan_oid], do_assert=False)
    elif attr == "SAI_VLAN_MEMBER_ATTR_BRIDGE_PORT_ID":
        status = npu.set(oid=sai_vlan_member[0], attr=[attr, npu.dot1q_bp_oids[0]], do_assert=False)
    assert status != "SAI_STATUS_SUCCESS"
